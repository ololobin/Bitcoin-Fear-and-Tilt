...
**Bitcoin Fear & Tilt** 是一款专为实时跟踪比特币价格动态和市场“恐慌与贪婪指数”而设计的 Android 应用程序。超越了传统的财务图表，它采用了**基于物理的动画**：一个拟物化的木制矿车会根据市场速度和价格变化方向做出动态反应。
...
- **基于物理的动画：** 界面通过弹性物理计算响应价格波动。视觉状态动态过渡以反映市场情绪的范围。
...
2. 下载最新编译的 `app-debug.apk`（生产版本即将在不久后提供）。
